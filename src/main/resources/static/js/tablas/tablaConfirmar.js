...
$('#citas tbody').on('click', '.btn-sm', function(){
  ...
  if (confirm('¿Estás seguro de que quieres confirmar todos los registros?')) {
    // Realiza una llamada AJAX para cambiar el estado de todos los registros de la BBDD
    $.ajax({
      url : '/confirmarTodo',
      method: 'PUT',
      success: function() {
       ...
       app.table.ajax.reload();
        $("#msg").text('Se actualizo los estados correctamente');
                $("#msg").show();
      },
      error: function(xhr, status, error) {
        alert('Error al confirmar todos los registros');
      }
    });
  }
};
